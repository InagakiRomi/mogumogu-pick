...
class RestaurantControllerTest {
    ...
    void testCreateRestaurant_validationFailed_returns400AndMessage() throws Exception {
        ...
        performPostRestaurants(invalidRequest)
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.result").value("error"))
                .andExpect(jsonPath("$.statusCode").value(400))
                .andExpect(jsonPath("$.code").value("BAD_REQUEST"))
                .andExpect(jsonPath("$.path").value("/restaurants"));
        assertMessageContains(performPostRestaurants(invalidRequest),
                "groupId must be greater than or equal to the minimum value");
        assertMessageContains(performPostRestaurants(invalidRequest), "categoryId is required");
        assertMessageContains(performPostRestaurants(invalidRequest), "restaurantName is required");
    ...
    void testDeleteRestaurant_success_returns200AndArchivedRestaurant() throws Exception {
        ...
        mockMvc.perform(delete("/restaurants/{id}", 10))
    ...
    void testUpdateRestaurant_validationFailed_returns400AndSkipsServiceCall() throws Exception {
        ...
        performPatchRestaurant(7, invalidRequest)
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.statusCode").value(400))
                .andExpect(jsonPath("$.code").value("BAD_REQUEST"));
        assertMessageContains(performPatchRestaurant(7, invalidRequest),
    ...
    private UpdateRestaurantDto buildUpdateRequest(String restaurantName, Integer selectedCount, Integer displayOrder) {
    ...
    private void assertMessageContains(ResultActions resultActions, String messagePart) throws Exception {
